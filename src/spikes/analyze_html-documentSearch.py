"""Analyze an HTML KB article using prebuilt-documentSearch.

Reads HTML from kb/1_html/<article>/, outputs markdown + JSON to kb/3_md/<article>/.
Also supports --url mode for direct URL-based analysis.
...
from azure.ai.contentunderstanding.models import AnalyzeResult, AnalyzeInput

from config import load_config, get_credential

KB_ROOT = Path(__file__).parent.parent / "kb"
HTML_DIR = KB_ROOT / "1_html"
MD_DIR = KB_ROOT / "3_md" / "analyze-html-documentSearch"
...
def analyze_from_url(url: str) -> AnalyzeResult:
    ...
    print(f"Mode: URL-based input")
    print(f"URL: {url}")
    print("-" * 60)

    poller = client.begin_analyze(
        analyzer_id=ANALYZER_ID,
        inputs=[AnalyzeInput(url=url)],
    )
...
def analyze_from_file(file_path: Path) -> AnalyzeResult:
    ...
    print(f"Analyzer: {ANALYZER_ID}")
    print(f"Mode: Binary upload (HTML)")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Analyze HTML KB articles with prebuilt-documentSearch"
    )
    ...
    group.add_argument(
        "--url",
        type=str,
        help="URL of an HTML page to analyze (CU fetches it directly)",
    )
    ...
    if args.url:
        ...
        output_dir = MD_DIR / "url-results"
        save_outputs(result, output_dir, slug or "url-result", "result")
    else:
        ...
        result = analyze_from_file(html_file)
        print_result(result)
