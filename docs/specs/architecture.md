...
- **Web Search Agent** — A specialist `Agent` for other Azure topics. Connects to an MCP web search server (SSE transport) that searches whitelisted sites (initially `learn.microsoft.com`). Uses `VisionImageMiddleware` only (no security filter or grounding — web results are public docs).
- **MCP Web Search Server** — A separate Container App exposing `web_search(query)` via MCP SSE transport. Dev mode uses fetch/scrape; prod mode uses Bing Grounding API via APIM.
