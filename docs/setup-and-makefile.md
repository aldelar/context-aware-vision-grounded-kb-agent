...
`system-check` reports the detected setup category (`macos-brew`, `linux-apt`, `linux-pacman`, or `linux-other`) plus the availability of Docker, Azure CLI, AZD, uv, Functions Core Tools, and npm. On macOS it also checks the native Ollama CLI; on Linux/WSL, Ollama is provided by Docker Compose and no host Ollama status line is printed.
