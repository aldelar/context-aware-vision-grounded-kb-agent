...
> This image is an architecture diagram that illustrates a secure, hybrid connectivity setup between on-premises infrastructure and Azure Cognitive Search using a [virtual network](https://learn.microsoft.com/en-us/azure/virtual-network/virtual-networks-overview). The flow shows how data moves from an on-premises environment through ExpressRoute (ER) Private Peering, passes an ER Gateway, and enters an App Service hosted within a virtual network (10.0.0.0/16). Internet access is specifically denied to this network segment. From the App Service, traffic is routed through a firewall before reaching the Azure Cognitive Search service, which operates within its own virtual network. The diagram emphasizes private connectivity and network security between on-premises and cloud services
...
> This image illustrates a secure network architecture for connecting on-premises resources to Azure Cognitive Search using Azure Private Link. The diagram shows traffic flowing from an on-premises environment through ExpressRoute Private Peering and an ExpressRoute Gateway into a virtual network. A virtual machine communicates with Azure Cognitive Search via a private endpoint, ensuring data flows privately without traversing the public internet. Both the virtual network and Azure Cognitive Search virtual network segments are configured to deny internet access, highlighting a secure, isolated connectivity setup
