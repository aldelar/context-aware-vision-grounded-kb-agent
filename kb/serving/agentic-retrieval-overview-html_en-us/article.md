...
> The image is a conceptual flowchart illustrating strategies for processing and understanding knowledge base queries. At the center is the example user query, "what does KB4048959 fix and what systems is the patch compatible with?" The diagram highlights three key aspects: using chat history for context, splitting queries to resolve co-references and extract specific information requirements, and performing spelling correction within the sentence context. Arrows indicate the relationships and data flow between the central query and the outlined processing strategies, emphasizing context-aware and nuanced query handling
...
> This image is a conceptual architecture diagram illustrating how agentic methods are applied to information retrieval in conversational AI systems. It shows the process starting from a user query and conversation turns feeding into a "Query planning" module. The planner then generates multiple search queries, each processed through two retrieval layers labeled L1 and L2. The results from these queries are then merged to provide a consolidated output. The diagram compares this multi-query, multi-stage agentic retrieval approach to a simpler method called a "Single LLM call," which handles the user input with context, spelling correction, query decomposition, and paraphrasing in a single step
