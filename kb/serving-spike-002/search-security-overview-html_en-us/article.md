...
> **Description**:  
This image illustrates a network architecture for securely connecting on-premises resources to Azure Cognitive Search using a private endpoint. It depicts a data flow where on-premises systems communicate through ExpressRoute private peering and an ExpressRoute (ER) Gateway to an application service within a [virtual network](https://learn.microsoft.com/en-us/azure/virtual-network/virtual-networks-overview). Internet access is denied to enforce security. The application service then connects privately to Azure Cognitive Search, also contained within its own virtual network, emphasizing secure, private connectivity without exposing endpoints to the public internet.
...
> **Description**:  
This image is a network architecture diagram demonstrating how to securely connect an on-premises environment to Azure Cognitive Search using Private Link. The flow starts from on-premises, passes through ExpressRoute private peering, an ER gateway, and a virtual machine, then connects to Azure over a private endpoint (with the IP 10.0.0.5). Internet access is denied at various stages, ensuring that communications between the on-premises network and Azure Cognitive Search remain private and secure within linked virtual networks.
